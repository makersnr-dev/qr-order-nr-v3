...
// 버전과 상관없이 동작하는 세션 미들웨어
app.use(async (req, res, next) => {
  try {
    req.session = await getIronSession(req, res, sessionOptions);
    // v8 스타일과 호환되도록 보강 (구버전도 동일 사용)
    req.session.save ??= async () => {};
    req.session.destroy ??= async () => {
      req.session = {};
      res.clearCookie(sessionOptions.cookieName, { path: '/' });
    };
    next();
  } catch (e) {
    next(e);
...
app.get('/store', (_req, res) =>
  res.sendFile(path.join(__dirname, '..', 'public', 'app', 'store.html'))
);
app.get('/delivery', (_req, res) =>
  res.sendFile(
    path.join(__dirname, '..', 'public', 'app', 'delivery-login.html')
  )
);
app.get('/delivery/home', (_req, res) =>
  res.sendFile(
    path.join(__dirname, '..', 'public', 'app', 'delivery-home.html')
  )
);
app.get('/payment/success', (_req, res) =>
  res.sendFile(path.join(__dirname, '..', 'public', 'app', 'success.html'))
);
app.get('/payment/fail', (_req, res) =>
  res.sendFile(path.join(__dirname, '..', 'public', 'app', 'fail.html'))
);
...
app.post('/orders', (_req, res) => {
  // allow customer order without auth
  const o = _req.body || {};
...
app.post('/confirm', requireAdmin, (_req, res) => {
  try {
    emitOrderEvent('confirmed', { ts: Date.now() });
  } catch (e) {}
  return res.json({ ok: true });
});
...
app.post('/payment/confirm', async (req, res) => {
  try {
    ...
    if (!paymentKey || !orderId || !amount) {
      return res
        .status(400)
        .json({ ok: false, message: 'paymentKey, orderId, amount required' });
    }
    const secretKey = process.env.TOSS_SECRET_KEY || '';
    if (!secretKey)
      return res.status(500).json({ ok: false, message: 'TOSS_SECRET_KEY missing' });
...
app.post('/import/menu', requireAdmin, upload.single('file'), (req, res) => {
  try {
    ...
    for (const r of json) {
      ...
      const active =
        String(r.active ?? r.Active ?? 'true').toLowerCase() !== 'false';
